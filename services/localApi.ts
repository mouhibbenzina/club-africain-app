...
class ApiClient {
  ...
  setToken(token: string) {
    this.token = token;
    if (token) {
      try { SecureStorage.setToken(token); } catch {}
    }
  }
  ...
  changePassword = (current_password: string, new_password: string) => this.post('/auth/change-password', { current_password, new_password });
  resendVerify = () => this.post('/auth/verify/resend');
  forgotPassword = (email: string) => this.post('/auth/forgot-password', { email });
  updateProfile = (data: { username?: string; full_name?: string; phone?: string; avatar?: string }) => this.put('/api/profiles', data);
  getMembers = (query?: string) => this.get(`/api/community/members${query?.trim() ? '?query=' + encodeURIComponent(query.trim()) : ''}`);
  getMember = (id: string) => this.get(`/api/community/members/${id}`);
  getFriends = () => this.get('/api/friends');
  getFriendRequests = () => this.get('/api/friends/requests');
  getSentRequests = () => this.get('/api/friends/sent');
  sendFriendRequest = (toUserId: string) => this.post('/api/friends/request', { to_user_id: toUserId });
  acceptFriendRequest = (requestId: number) => this.post('/api/friends/accept', { request_id: requestId });
  declineFriendRequest = (requestId: number) => this.post('/api/friends/decline', { request_id: requestId });
  cancelFriendRequest = (userId: string) => this.post('/api/friends/cancel', { user_id: userId });
  unfriend = (userId: string) => this.request(`/api/friends/${userId}`, { method: 'DELETE' });
  getAdminStats = () => this.get('/api/admin/stats');
  getAdminUsers = (query?: string) => this.get(`/api/admin/users${query?.trim() ? '?query=' + encodeURIComponent(query.trim()) : ''}`);
  setUserRole = (id: string, role: 'fan' | 'vip' | 'admin') => this.put(`/api/admin/users/${id}/role`, { role });
  getAdminNews = () => this.get('/api/admin/news');
  createAdminNews = (data: { sport_id?: number; title: string; excerpt?: string; content?: string; image_url?: string; published?: boolean }) => this.post('/api/admin/news', data);
  updateAdminNews = (id: number, data: { sport_id?: number; title?: string; excerpt?: string; content?: string; image_url?: string; published?: boolean }) => this.put(`/api/admin/news/${id}`, data);
  deleteAdminNews = (id: number) => this.request(`/api/admin/news/${id}`, { method: 'DELETE' });
  getAdminMatches = () => this.get('/api/admin/matches');
  createAdminMatch = (data: { sport_id?: number; home_team: string; away_team: string; date: string; venue?: string; competition?: string; status?: string; home_score?: number | null; away_score?: number | null }) => this.post('/api/admin/matches', data);
  updateAdminMatch = (id: number, data: { sport_id?: number; home_team?: string; away_team?: string; date?: string; venue?: string; competition?: string; status?: string; home_score?: number | null; away_score?: number | null }) => this.put(`/api/admin/matches/${id}`, data);
