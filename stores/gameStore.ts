...
interface GameState {
  ...
  isLoading: boolean;
  error: string | null;
  submitPrediction: (matchId: number, home: number, away: number) => Promise<void>;
  saveTeam: (formation: string, players: string[]) => Promise<void>;
  loadTeam: () => Promise<void>;
  votePlayer: (matchId: number, player: string) => Promise<void>;
}
...
export const useGameStore = create<GameState>((set) => ({
  ...
  submitPrediction: async (matchId, home, away) => {
    set({ error: null });
    try {
  ...
  saveTeam: async (formation, players) => {
    set({ error: null });
    try {
  ...
  votePlayer: async (matchId, player) => {
    set({ error: null });
    try {
